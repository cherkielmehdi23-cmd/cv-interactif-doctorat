...
export default function Home({ locale }: { locale: Locale }) {
  ...
  return <div className="archive-home">
    <section className="archive-hero"><div className="hero-copy"><p className="crumb">01 / {t.archive}</p><h1>Dossier doctoral<br /><em>structuré.</em></h1><p>Le CV structure les intitulés, le parcours et les expériences ; l’archive ZIP apporte les diplômes, attestations, lettres, travaux scientifiques et pièces de contrôle correspondantes.</p><div className="hero-actions"><Link href="/registre-des-pieces"><FileText size={16} /> {t.register}</Link><Link className="outline" href="/confidentialite"><ShieldCheck size={16} /> {t.nav.privacy}</Link></div></div><div className="hero-image"><img src={assets.hero} alt="Dossier et pièces documentaires" /><div><span>CV / STRUCTURATION</span><span>ZIP / PREUVES</span></div></div></section>
    ...
    <section className="home-intro"><div><p className="crumb">02 / {t.coverage}</p><h2>Un parcours professionnel, des preuves organisées, une source explicitée.</h2></div><p>Les informations de parcours, les stages et les activités sont reformulés à partir des CV français et anglais. Chaque notice précise ensuite si elle est soutenue par un justificatif du ZIP ou si son document original reste à ajouter.</p></section>
