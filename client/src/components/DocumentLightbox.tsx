...
export default function DocumentLightbox({ preview, locale, onClose, onPrevious, onNext, hasPrevious, hasNext }: { preview: DocumentPreview | null; locale: Locale; onClose: () => void; onPrevious?: () => void; onNext?: () => void; hasPrevious?: boolean; hasNext?: boolean }) {
  ...
  useEffect(() => { setZoom(1); }, [preview?.id, preview?.src]);
  useEffect(() => {
  ...
  return <Dialog open={Boolean(preview)} onOpenChange={open => { if (!open) onClose(); }}>
    <DialogContent className="document-lightbox max-w-5xl p-0 overflow-hidden border-0">
      {preview && <><DialogHeader className="lightbox-head"><div><DialogTitle>{preview.title}</DialogTitle><DialogDescription><FileCheck2 size={14} /> {preview.source}</DialogDescription></div><span><ShieldCheck size={14} /> {copy.zip}</span></DialogHeader><div className="lightbox-zoombar"><span><Maximize2 size={14} /> {Math.round(zoom * 100)}%</span><div><button onClick={() => setZoom(value => clampZoom(value - .25))} disabled={zoom <= 1} aria-label="Réduire le zoom"><Minus size={15} /></button><button onClick={() => setZoom(value => clampZoom(value + .25))} disabled={zoom >= 3} aria-label="Augmenter le zoom"><Plus size={15} /></button><button className="zoom-reset" onClick={() => setZoom(1)} disabled={zoom === 1}>100%</button></div></div><div className={zoom > 1 ? "lightbox-image-wrap is-zoomed" : "lightbox-image-wrap"} onWheel={event => { event.preventDefault(); setZoom(value => clampZoom(value + (event.deltaY < 0 ? .2 : -.2))); }}><button className="lightbox-nav previous" onClick={onPrevious} disabled={!hasPrevious} aria-label="Document précédent"><ChevronLeft size={22} /></button><div className="lightbox-viewport"><img key={preview.id || preview.src} className="lightbox-asset" style={{ transform:`scale(${zoom})` }} src={preview.src} alt={preview.title} onDoubleClick={() => setZoom(value => value === 1 ? 2 : 1)} /></div><button className="lightbox-nav next" onClick={onNext} disabled={!hasNext} aria-label="Document suivant"><ChevronRight size={22} /></button></div><div className="lightbox-download">{preview.download && <a href={preview.download} download><Download size={15} /> {locale === "fr" ? "Télécharger le document" : "Download document"}</a>}<span>{locale === "fr" ? "Molette, double-clic ou + / − pour zoomer" : "Use wheel, double-click or + / − to zoom"}</span></div></>}
    </DialogContent>
  </Dialog>;
}
